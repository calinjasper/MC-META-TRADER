"""
Structure-Based Trading Strategy

This strategy follows the project structure guidelines from PROJECT_STRUCTURE.md:
- Located in src/strategy/ directory
- Inherits from BaseStrategy
- Uses organized, modular code structure
- Implements Smart Money Concepts (SMC) with improved structure

Key improvements over original SMC strategy:
...
@dataclass
class Pivot:
    """Represents a pivot point in price action"""
    kind: str  # "high" | "low"
    price: float
    index: int
...
def _extract_ohlc_arrays(rates: List[Dict]) -> Tuple[List[float], List[float], List[float], List[float]]:
    """Extract OHLC arrays from rate data"""
...
class StructureStrategy(BaseStrategy):
    ...
    This strategy implements Smart Money Concepts (SMC) following the project structure:
    - Tracks swing/internal pivots (fractal pivots)
    - Detects BOS (Break of Structure) / CHoCH (Change of Character) based on pivot breaks
    - Maintains state machine (bias + last structure levels)
    - Emits BUY/SELL signals based on structure changes
    ...
    def __init__(
        ...
        self.entry_conditions_text = [f"Structure: emit={self.emit_on}, pivots={self.pivot_left}/{self.pivot_right}"]
    ...
    def _get_candles(self, count: int = 300) -> List[Dict]:
        ...
        if self.mt5_connector and hasattr(self.mt5_connector, "is_connected") and self.mt5_connector.is_connected():
            try:
                ...
                logger.error(f"StructureStrategy {self.name}: Error fetching candles via MT5Connector: {e}", exc_info=True)
                return []
        return []
    ...
    def _resolve_operand(self, field: str, current_price: float, ohlc: Dict, market_data: Dict = None) -> Optional[float]:
        ...
        Falls back to internal structure pivot tracking if unified resolver doesn't have pivots.
        """
        ...
        # Add structure pivots from internal state (if not in market_data)
        structure_pivots = {}
        if self.last_pivot_high:
            structure_pivots["pivot_high"] = self.last_pivot_high.price
        if self.last_pivot_low:
            structure_pivots["pivot_low"] = self.last_pivot_low.price
        context["smc_pivots"] = structure_pivots  # Keep compatibility with SMC naming
    ...
    def _evaluate_filter_chain_with_previous(self, filters: List[Dict], current_price: float, ohlc: Dict, market_data: Dict = None, previous_price: Optional[float] = None) -> bool:
        """Evaluate filter chain with previous price for cross detection"""
        if not filters:
            return True
        cumulative = None
        prev_connector = None
    ...
    def generate_signal(self, market_data: Dict) -> Optional[str]:
        """Generate trading signal based on structure analysis"""
        # Candle source
    ...
    def to_dict(self) -> Dict:
        ...
        base.update(
            {
                "strategy_type": "structure",  # Changed from "smc" to "structure"
    ...
    @classmethod
    def from_dict(cls, data: Dict, mt5_connector=None) -> "StructureStrategy":
        ...
        strategy.entry_conditions_text = [f"Structure: emit={strategy.emit_on}, pivots={strategy.pivot_left}/{strategy.pivot_right}"]
