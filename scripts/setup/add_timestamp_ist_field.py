"""
Add timestamp_ist Field to Ticks Collection
Adds timestamp_ist field to existing ticks collection schema for IST-formatted display
"""
...
def add_timestamp_ist_field(token):
    """Add timestamp_ist field to ticks collection if it doesn't exist"""
    headers = {"Authorization": f"Bearer {token}"}
    ...
    if has_timestamp_ist:
        print("[OK] 'ticks' collection already has 'timestamp_ist' field")
        return True
    ...
    try:
        ...
        if response.status_code == 200:
            print("[OK] Added 'timestamp_ist' field to 'ticks' collection")
            return True
        else:
            print(f"[ERROR] Failed to update ticks collection: Status {response.status_code}")
...
def main():
    print("=" * 70)
    print("  Add timestamp_ist Field to Ticks Collection")
    print("=" * 70)
    print()
    ...
    print("Adding 'timestamp_ist' field to 'ticks' collection...")
    if add_timestamp_ist_field(token):
        ...
        print("=" * 70)
        print()
        print("The 'ticks' collection now has the 'timestamp_ist' field.")
        ...
        print("2. New ticks will include both 'timestamp' (milliseconds) and 'timestamp_ist' (IST format)")
